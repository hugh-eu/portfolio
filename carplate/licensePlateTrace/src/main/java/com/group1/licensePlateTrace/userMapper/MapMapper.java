...
public interface MapMapper {
	...
	/* 전체정보 조회 */
	@Select("SELECT * FROM "
			+ "tbl_license_plate_info l "
			+ "JOIN tbl_license_plate_search s "
			+ "ON l.l_info = s.s_name "
			+ "JOIN tbl_camera c "
			+ "ORDER BY l.l_info_reg_date ASC LIMIT 50")
	...
	@Select("SELECT s.s_reg_date, "
			...
			+ "AND YEAR(l.l_info_reg_date) = l.l_info_reg_date ORDER BY l.l_info_reg_date ASC")
	...
	@Select("SELECT s.s_reg_date, l.l_info_reg_date, l.l_info, s.s_name "
			...
			+ "ORDER BY l.l_info_reg_date ASC")
	...
	@Select("SELECT  * FROM tbl_license_plate_search s "
			...
			+ "AND l.c_no = #{c_no} "
			+ "ORDER BY l_info_reg_date ASC;")
	public Map<String, MapVo> getDetailItems(@Param("l_no") int i,
											 @Param("c_no") int j);

      ...
      $.ajax({
         ...
         success: function(data) {
            ...
            for (let i = 0; i < mapVos.length; i++){
               ...
               console.log('l_info_reg_date----> ' + l_info_reg_date);
               console.log('l_info_reg_date----> ' + l_info_reg_date.length);
               
               $('#' + i + ' td.date span.year').text(l_info_reg_date.substr(0, 4).replace(/-/g, ""));
               $('#' + i + ' td.date span.month').text(l_info_reg_date.substr(5, 3).replace(/-/g, ""));
               let day = '';
               if (l_info_reg_date.length < 19) { 
                  day = l_info_reg_date.substring(6, 9).replace(/-/g, "");
                  console.log(day)
               }else {
                  day = l_info_reg_date.substring(7, 10).replace(/-/g, "");
                  console.log(day)
               }
               $('#' + i + ' td.date span.date').text(day);
               $('#' + i + ' td.date span.c_no').text("카메라 고유 번호: " + c_no);

               $('#' + i + ' td.plate_info a.name').text(s_name);
               $('#' + i + ' td.plate_info a.info').text(s_info);

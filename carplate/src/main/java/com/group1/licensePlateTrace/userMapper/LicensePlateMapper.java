...
public interface LicensePlateMapper {
	...
	@Select("SELECT * FROM tbl_license_plate_search ORDER BY s_reg_date ASC LIMIT #{skip}, #{amount}")
	List<LicensePlateVo> selectAllLicensePlateASC(@Param("skip") int skip, 
												  @Param("amount") int amount);

	@Select("SELECT * FROM tbl_license_plate_search WHERE s_name = #{licensePlateName} ORDER BY s_reg_date DESC LIMIT #{skip}, #{amount}")
	List<LicensePlateVo> selectAllLicensePlateDESCByName(@Param("skip") int skip, 
														 @Param("amount") int amount, 
														 @Param("licensePlateName") String licensePlateName);
	
	@Select("SELECT * FROM tbl_license_plate_search WHERE s_name = #{licensePlateName} ORDER BY s_reg_date ASC LIMIT #{skip}, #{amount}")
